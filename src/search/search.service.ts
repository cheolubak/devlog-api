...
import { FeedType, Users } from '../database/generated/prisma';
...
export class SearchService {
  ...
  async searchBookmarks({
    ...
    return await this.searchBookmarkPosts(
      pattern,
      user.id,
      limit,
      skip,
  ...
  private async searchBookmarkPosts(
    pattern: string,
    userId: string,
    limit: number,
    skip: number,
    ...
    const [matchedIds, totalResult] = await Promise.all([
      this.prisma.$queryRaw<{ id: string }[]>`
        SELECT p2.id
        FROM "Posts" p2
               INNER JOIN "post_bookmarks" pb ON pb."post_id" = p2.id AND pb."user_id" = ${userId}::uuid
